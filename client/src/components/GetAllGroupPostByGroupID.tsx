...
const GET_ALL_PUBLIC_POST = gql`
  query SearchAllGroupPost($search:String!, $offset: Int, $limit: Int){
    searchAllGroupPost(search:$search, offset:$offset, limit:$limit){
      id
      groupID
      groupName
      userID
...
const GetAllGroupPostByGroupID = ({user, search, group_id}) => {
  ...
  const { data, loading, error, fetchMore } = useQuery(GET_ALL_PUBLIC_POST, {
    variables: { search: "", limit: 3, offset: 0 },
  });

  console.log(user);
  console.log(search);
  
  const items = data?.searchAllGroupPost || []; 
  console.log(items);
  ...
  window.onscroll = async function (ev) {
    if (window.innerHeight + window.scrollY >= document.body.offsetHeight - 100) {
      ...
      if (items.length < itemsLoaded) {
        ...
        await fetchMore({
          variables: { search: "", limit: 3, offset: itemsLoaded },
          updateQuery: (prev, { fetchMoreResult }) => {
            if (!fetchMoreResult) return prev;
            return {
              searchAllGroupPost: [...prev.searchAllGroupPost, ...fetchMoreResult.searchAllGroupPost],
            };
          },
        });
      }
  ...
  const clickGroupName = (groupID) => {
    navigate('/group/profile', { state: { value: { groupID } } });
    console.log(groupID)
  }
  const filteredItems = items.filter(item => item.groupID === group_id);
            ...
            <div className="postComponent">
              ...
              <div className="postHeader">
                ...
                <div className="userInfo">
                  <h4 onClick={(e) => clickGroupName(item.groupID)}>{item.groupName}</h4>
                  {item.privacy === 'public' && (
                    <div className="privacy-info" style={{width:'max-content'}}>
                      <h6 style={{marginRight:'5px'}}>{item.userName} </h6> &#183;
                      <h6 className="privacy-text" style={{marginLeft:'5px'}}>{formatDistanceToNow(new Date(item.datePost))} ago</h6>
...
export default GetAllGroupPostByGroupID;
